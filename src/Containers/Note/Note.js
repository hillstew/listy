...
// FUNCTIONAL
// const toggleIssueCompletion = (e, issues, id, title, color) => {
//   const index = getIndex(e.target.id, issues);
//   const newIssues = createIssuesCopy(issues);
//   newIssues[index].completed = !newIssues[index].completed;
...
// const renderIssues = (completed, issues, id, title, color) => {
//   const iconClass = completed ? 'fas fa-check-square': 'fas fa-square'; 
...
//           <i
//             className={iconClass}
//             onClick={(e) => toggleIssueCompletion(e, issues, id, title, color)}
//             id={issue.id}
//           />
//           {issue.body}
//         </div>
...
//   const incompleteIssues = renderIssues(false, note.issues, note.id, note.title, note.color);
//   const completeIssues = renderIssues(true, note.issues, note.id, note.title, note.color)
//   return (
//     <div className={`note-card ${note.color}`}>
//       <h3>{note.title}</h3>
//       <ul className='incomplete-list'>{incompleteIssues}</ul>
//       <ul className='completed-list'>{completeIssues}</ul>
